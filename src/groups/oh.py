...
def _generate_rotations() -> list[np.ndarray]:
    ...
    rest = [matrix for _, matrix in sorted(unique.items()) if not np.array_equal(matrix, identity)]
...
def _build_cayley_table(rotations: list[np.ndarray]) -> np.ndarray:
    ...
    return np.asarray(
        [[index[tuple((left @ right).ravel())] for right in rotations] for left in rotations],
        dtype=np.int64,
    )
