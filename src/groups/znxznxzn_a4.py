...
def _regular_irreps_from_cayley(
    cayley: np.ndarray, classification: str
) -> list[LittleIrrep]:
    ...
    for h in range(size):
        ...
        operator += (
            coefficient * right[h]
            + coefficient.conjugate() * right[h].conj().T
        )
    ...
    for idx, value in enumerate(eigenvalues):
        if (
            not clusters
            or abs(value - eigenvalues[clusters[-1][-1]]) > 1e-8
        ):
            clusters.append([idx])
        else:
    ...
    for cluster in clusters:
        ...
        if any(
            np.allclose(characters, seen, atol=1e-7)
            for seen in characters_seen
        ):
            continue
    ...
    if dim_sum != size:
        raise ValueError(
            f"Failed to decompose {classification}: "
            f"sum dim^2={dim_sum}, subgroup size={size}"
        )
...
class DiscreteSE3A4Group(Group):
    ...
    def inverse(self, g: int) -> int:
        ...
        x_inverse, y_inverse, z_inverse = self._apply_rotation(
            rotation_inverse, -x, -y, -z
        )
        return self.encode(
            x_inverse, y_inverse, z_inverse, rotation_inverse
        )
    ...
    def compose(self, g: int, h: int) -> int:
        ...
        x2_rotated, y2_rotated, z2_rotated = self._apply_rotation(
            rotation1, x2, y2, z2
        )
    ...
    def left_action(self, g: int, signal: np.ndarray) -> np.ndarray:
        ...
        if signal.shape[-1] != self.order:
            raise ValueError(
                f"signal final axis must have length {self.order}, "
                f"got {signal.shape}"
            )
    ...
    def _build_rotation_cayley(self) -> np.ndarray:
        index = {
            tuple(matrix.ravel()): idx
            for idx, matrix in enumerate(self._rot_mats)
        }
        cayley = np.empty(
            (_ROTATION_ORDER, _ROTATION_ORDER), dtype=np.int64
        )
    ...
    def _build_rotation_inverses(self) -> np.ndarray:
        ...
        for rotation in range(_ROTATION_ORDER):
            ...
            if len(matches) != 1:
                raise ValueError(
                    f"could not find inverse for rotation {rotation}"
                )
            inverses[rotation] = int(matches[0])
        return inverses

    def _dual_action(
        self, rotation: int, character: tuple[int, int, int]
    ) -> tuple[int, int, int]:
        vector = np.array(character, dtype=int)
        return tuple(
            (
                self._rot_mats[int(rotation)] @ vector % self._n
            ).tolist()
        )
    ...
    def _compute_orbit_data(self) -> list[dict[str, Any]]:
        ...
        for character in product(range(self._n), repeat=3):
            ...
            orbit = sorted(
                {
                    self._dual_action(rotation, character)
                    for rotation in range(_ROTATION_ORDER)
                }
            )
            ...
            stabilizer = tuple(
                rotation
                for rotation in range(_ROTATION_ORDER)
                if self._dual_action(rotation, representative)
                == representative
            )
            ...
            coset_reps, orbit_labels, transition = self._coset_data(
                representative, stabilizer
            )
            data.append(
                {
    ...
    def _coset_data(
        ...
        orbit_labels = [
            self._dual_action(coset_rep, representative)
            for coset_rep in coset_reps
        ]
        if len(set(orbit_labels)) != len(orbit_labels):
            raise ValueError(
                "Coset reps do not map bijectively onto orbit for "
                f"{representative}"
            )

        coset_lookup = {}
        stabilizer_lookup = {
            stabilizer_element: idx
            for idx, stabilizer_element in enumerate(stabilizer)
        }
        for i, coset_rep in enumerate(coset_reps):
            for stabilizer_element in stabilizer:
                coset_lookup[
                    int(self._rot_cayley[coset_rep, stabilizer_element])
                ] = (i, stabilizer_lookup[stabilizer_element])

        transition = np.empty(
            (_ROTATION_ORDER, len(coset_reps), 2), dtype=np.int64
        )
        for rotation in range(_ROTATION_ORDER):
            for j, coset_rep in enumerate(coset_reps):
                transition[rotation, j] = coset_lookup[
                    int(self._rot_cayley[rotation, coset_rep])
                ]
    ...
    def _subgroup_cayley(self, stabilizer: tuple[int, ...]) -> np.ndarray:
        local = {
            rotation: idx for idx, rotation in enumerate(stabilizer)
        }
        cayley = np.empty(
            (len(stabilizer), len(stabilizer)), dtype=np.int64
        )
    ...
    def _little_irreps(
        ...
        if stabilizer not in self._little_irrep_cache:
            ...
            self._little_irrep_cache[
                stabilizer
            ] = _regular_irreps_from_cayley(cayley, classification)
    ...
    def _build_irreps(self) -> list[LazyIrreducibleRepresentation]:
        ...
        for orbit_idx, data in enumerate(self._orbit_data):
            ...
            for sigma_idx, sigma in enumerate(little_irreps):
                irreps.append(
                    self._make_induced_irrep(
                        orbit_idx, data, sigma_idx, sigma
                    )
                )
        dim_sum = sum(irrep.dim**2 for irrep in irreps)
        if dim_sum != self.order:
            raise ValueError(
                f"Peter-Weyl dimension sum failed: "
                f"{dim_sum} != {self.order}"
            )
        return irreps
    ...
    def _make_induced_irrep(
        ...
        def matrix_fn(element_index: int) -> np.ndarray:
            ...
            for j in range(orbit_size):
                ...
                phase_arg = (
                    int(np.dot(orbit_labels[i], translation)) % self._n
                )
                character = np.exp(
                    2j * np.pi * phase_arg / self._n
                )
                row = slice(i * sigma_dim, (i + 1) * sigma_dim)
                column = slice(j * sigma_dim, (j + 1) * sigma_dim)
                matrix[row, column] = character * sigma(
                    int(stabilizer_idx)
                )
            return matrix
        ...
        irrep = LazyIrreducibleRepresentation(
            name, dim, matrix_fn, cache_size=128
        )
        irrep._metadata = {
            "orbit_idx": orbit_idx,
    ...
    def _build_conjugate_pairs(self) -> list[list[int]]:
        ...
        characters = [
            self._character_vector(irrep, elements) for irrep in irreps
        ]
        ...
        for i, character in enumerate(characters):
            ...
            matches = [
                ...
                and np.allclose(
                    candidate, character.conjugate(), atol=1e-7
                )
            ]
            if not matches:
                raise ValueError(
                    f"Could not find conjugate irrep for index {i}"
                )
            j = matches[0]
            processed.add(i)
            processed.add(j)
