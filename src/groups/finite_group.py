...
def irreps_from_regular(
    ...
    for element in range(order):
        ...
        operator += (
            coefficient * right[element]
            + coefficient.conjugate() * right[element].conj().T
        )
    ...
    for cluster in clusters:
        ...
        matrices = np.asarray(
            [basis.conj().T @ left[element] @ basis for element in range(order)]
        )
...
def _inverses(cayley: np.ndarray) -> np.ndarray:
    ...
    for element in range(order):
        matches = np.flatnonzero(
            (cayley[element] == 0) & (cayley[:, element] == 0)
        )
        if len(matches) != 1:
...
def _order_by_dimensions(
    ...
    for dimension in dimension_order:
        match = next(
            (
                index
                for index, matrices in enumerate(remaining)
                if matrices.shape[1] == dimension
            ),
            None,
        )
        if match is None:
