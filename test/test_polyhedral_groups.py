...
def test_group_operations(group):
    ...
    assert all(group.compose(element, group.inverse(element)) == 0 for element in elements)
    assert all(group.compose(group.inverse(element), element) == 0 for element in elements)
...
def test_irrep_characters_are_orthonormal(group):
    characters = np.asarray(
        [[np.trace(irrep(element)) for element in group.elements()] for irrep in group.irreps()]
    )
