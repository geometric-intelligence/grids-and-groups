...
class TestTetrahedralRotations:
    def test_rotations_form_a4(self):
        ...
        matrices = [
            group.rotation_matrix(rotation)
    ...
    def test_rotations_preserve_tetrahedron(self):
        ...
        for rotation in range(group.num_rotations):
            ...
            images = {
                tuple((matrix @ np.asarray(vertex)).tolist())
...
class TestConstructionAndOrbits:
    ...
    def test_irreps_are_lazy(self):
        group = DiscreteSE3A4Group(2)
        assert all(
            isinstance(irrep, LazyIrreducibleRepresentation)
...
class TestLittleGroups:
    ...
    def test_little_group_irreps(self, n):
        ...
        for data in group.orbit_data():
            ...
            assert sum(irrep.dim**2 for irrep in little_irreps) == len(
                stabilizer
            )

            for irrep in little_irreps:
                np.testing.assert_allclose(
                    irrep(0), np.eye(irrep.dim), atol=1e-8
                )
...
class TestInducedIrreps:
    ...
    def test_identity_is_identity_matrix(self):
        ...
        for irrep in group.irreps():
            np.testing.assert_allclose(
                irrep(group.identity()), np.eye(irrep.dim), atol=1e-8
            )

    def test_unitarity_sample(self):
        group = DiscreteSE3A4Group(2)
        elements = np.random.default_rng(0).integers(
            0, group.order, size=12
        )
        for irrep in group.irreps():
    ...
    def test_homomorphism_sample(self):
        group = DiscreteSE3A4Group(2)
        pairs = np.random.default_rng(1).integers(
            0, group.order, size=(16, 2)
        )
        for irrep in group.irreps():
            for a, b in pairs:
                a, b = int(a), int(b)
...
class TestActions:
    ...
    def test_left_actions_compose(self):
        ...
        np.testing.assert_allclose(
            group.left_action(
                second, group.left_action(first, signal)
            ),
...
class TestFourier:
    def test_fourier_roundtrip(self):
        ...
        np.testing.assert_allclose(
            reconstructed.real, signal, atol=1e-10
        )
        np.testing.assert_allclose(
            reconstructed.imag, np.zeros(group.order), atol=1e-10
        )
    ...
    def test_parseval(self):
        ...
        lhs = sum(
            irrep.dim
            * np.real(
                np.trace(coefficient.conj().T @ coefficient)
            )
            for irrep, coefficient in zip(
                group.irreps(), coefficients
            )
        )
        ...
        for pair in group.conjugate_pairs():
            if len(pair) == 1:
                ...
                for element in group.elements():
                    ...
                    assert character == pytest.approx(
                        character.conjugate(), abs=1e-8
                    )
            else:
                i, j = pair
                for element in group.elements():
