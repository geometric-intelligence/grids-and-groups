...
def _exact_cnxcn_model():
    ...
    return group, model, x_allo


def test_fixed_point_embedding_converges_for_exact_model():
    group, model, x_allo = _exact_cnxcn_model()
    initial_states = model.probe_hidden_states(x_allo)

    fixed = fixed_point_embedding(model, initial_states)

    assert fixed.converged
    assert fixed.iterations == 1
    assert np.max(fixed.residuals) < 1e-10
    assert fixed.states.shape == (group.order, model.hidden_dim)


def test_conjugate_irreps_are_combined_into_real_modules():
    _, model, _ = _exact_cnxcn_model()

    groups = conjugate_irrep_groups(model)
...
def test_module_orbits_and_analysis_have_expected_shapes():
    group, model, x_allo = _exact_cnxcn_model()
    hidden = model.probe_hidden_states(x_allo)
    fixed = fixed_point_embedding(model, hidden)
    modules = build_module_orbits(model, fixed.states)
...
def test_module_orbit_analysis_supports_three_dimensional_umap():
    ...
    hidden = model.probe_hidden_states(x_allo)
    fixed = fixed_point_embedding(model, hidden)
    module = build_module_orbits(model, fixed.states)[0]
