...
from src.groups import DiscreteSE3A4Group
...
#
# Write an element as $g=(t,r)$, with $t\in\mathbb{Z}_n^3$ and $r\in A_4$. If $R_r$ is the signed permutation matrix for rotation $r$, then
#
...
#
# Irreps are constructed by the little-group method: tetrahedral rotations act on translation frequencies $k\in\mathbb{Z}_n^3$, and stabilizer irreps are induced to the full semidirect product. The implementation checks $\sum_\rho d_\rho^2=|G|$.
...
def rotation_index(group, matrix):
    """Look up a tetrahedral rotation by its implemented allocentric matrix."""
...
#
# The allocentric code is an off-center anisotropic periodic Gaussian with a peaked tetrahedral-orientation profile, making both translation and rotation observable. The egocentric code is random and Fourier-invertible on every selected nonabelian block.
#
# Orthogonal slices summarize translation, a 12-bin marginal summarizes orientation, and RMS energy avoids cancellation for signed signals. These are compact diagnostics of the full signal on $\mathbb Z_n^3\rtimes A_4$.

# %%
G = DiscreteSE3A4Group(n=3)
...
ax.set(
    xlabel="Tetrahedral rotation index",
...
figure.suptitle("Closed-form tetrahedral SE(3) RNN weights; lines mark irrep blocks")
plt.show()
...
rotation_cycle_xyz = rotation_index(
    G,
    [[0, 0, 1], [1, 0, 0], [0, 1, 0]],
)
rotation_cycle_xzy = rotation_index(
    G,
    [[0, 1, 0], [0, 0, 1], [1, 0, 0]],
)
...
for _ in range(num_long_steps):
    ...
    if draw < 0.05:
        ...
        rotation = rotation_cycle_xyz if rng_long.random() < 0.5 else rotation_cycle_xzy
...
plot_pose_trajectory(
    ...
    title="Exact and decoded tetrahedral SE(3) pose",
)
plt.show()
...
#
# A full tuning curve lives on 324 poses and cannot be displayed in one ordinary plot. We therefore use two complementary projections:
#
...
# 2. translation coordinates are summed, producing a representative-neuron by 12-orientation heatmap.
#
...
#
# 1. **translation probe:** fix tetrahedral orientation at the identity and vary all $3^3=27$ positions;
# 2. **orientation probe:** fix translation at the origin and vary all 12 tetrahedral rotations.
#
...
#
# These results are necessarily exploratory. Twenty-seven spatial samples are far too sparse to establish the topology of a continuous three-torus, and the 12-element tetrahedral rotation group is a finite noncommutative set—not a densely sampled copy of $SO(3)$ or a circle. The two probes are plotted separately precisely because their topology and interpretation differ.
...
translation_analyses = analyze_module_orbits(
    translation_modules,
    max_persistence_points=len(translation_elements),
    max_homology_dimension=2,
    random_state=13,
)
orientation_analyses = analyze_module_orbits(
    orientation_modules,
    max_persistence_points=len(orientation_elements),
    max_homology_dimension=2,
    random_state=17,
)
...
for analysis in orientation_analyses:
    plot_manifold_analysis(
        analysis,
        orientation_colors,
        title=f"Finite tetrahedral orientation probe — {analysis.module.label}",
    )
    plt.show()
...
# ## 8. Summary
#
# - The parameterized tetrahedral discrete-$SE(3)$ model uses induced matrix-valued irreps, cost-aware power selection, and factored recurrent mixing.
# - Position and tetrahedral orientation are both observable; the primary rollout reports signal, periodic-position, and geodesic-rotation errors.
