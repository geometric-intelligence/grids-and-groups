...
def powers_per_neuron_rows(W: np.ndarray, group) -> np.ndarray:
    ...
    The absolute scale differs from a normalized FFT by a factor of ``|G|``,
    but downstream dominant-mode plots use ratios, so this does not affect
    their values.
...
def model_power_over_time(group, model, param_history, model_inputs):
    ...
    Uses ``group.power_spectrum`` uniformly for every supported group.
...
def plot_predictions_group(
    ...
    Uses a bar-chart representation that works for any group order.
    """
    import torch
...
def _hidden_by_group_weights_from_state_dict(sd: dict) -> np.ndarray | None:
    """Return ``W_out.T`` with shape ``(hidden, group_dim)`` when present."""
    if "W_out" in sd:
...
def mode_colors_aligned_with_power_plot(
    ...
    """One color per mode index 0..n_modes-1, matching :func:`plot_power_group`.
...
def plot_w_dominant_irrep_fraction(
    ...
    **What you see**
    ...
    - **Each colored line is one hidden neuron** (one column of ``W_out`` turned into a row).
      Lines use the color of the **mode or irrep** that neuron ends up favoring (see below).
    ...
    **What “power” and “fraction” mean**
    ...
    Each neuron's weights form a vector on the group (length ``group.order``). That vector is decomposed
    into an irrep-wise **power spectrum** via ``group.power_spectrum``. **Total power** is the
    ...
    **How to read the figure**
    ...
    Args:
        ...
        group: Finite group matching the second axis of ``W_out.T``
    ...
    Returns:
        ``matplotlib.figure.Figure`` or ``None`` if the model has no ``W_out`` parameter
