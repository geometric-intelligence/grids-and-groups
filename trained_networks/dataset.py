...
class GroupCompositionDataset(Dataset):
    ...
    **kwargs
        ...
        - offline: ``template, k, mode, num_samples, return_all_outputs, action_side``
    ...
    def __new__(cls, group, *, online=False, action_side="right", **kwargs):
    ...
    def __init__(self, group, *, online=False, action_side="right", **kwargs):
        if online:
            return
        super().__init__()
        X, Y, sequence = self._build_group(
            group=group, action_side=action_side, **kwargs
        )
    ...
    def _build_group(
        ...
        ``CyclicGroup`` and ``ProductCyclicGroup`` use the same group-generic
        construction as every other supported finite group.
        """
        ...
        rep_matrices = action_group.regular_rep()
