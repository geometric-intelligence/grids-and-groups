...
def _save_param_history_pt(param_hist: list, path: Path) -> None:
    """Atomically write ``param_history.pt``."""
    path = Path(path)
...
def regenerate_plots(run_dir, device="cpu"):
    ...
    param_save_indices = np.load(run_dir / "param_save_indices.npy").tolist()
...
def train_single_run(config: dict, run_dir: Path = None) -> dict:
    ...
    if training_mode == "online":
        from trained_networks import train as train_mod
...
GROUP_CONFIG_MAP = {
    "C11": "trained_networks/configs/config_c11.yaml",
    "C5xC5": "trained_networks/configs/config_c5xc5.yaml",
    "D5": "trained_networks/configs/config_d5.yaml",
    "Oh": "trained_networks/configs/config_oh.yaml",
    "A5": "trained_networks/configs/config_a5.yaml",
}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="trained_networks/configs/config.yaml",
        help="Path to config YAML file (default: trained_networks/configs/config.yaml)",
    )
    parser.add_argument(
        "--regenerate",
        type=str,
