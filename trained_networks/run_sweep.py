#!/usr/bin/env python3
"""
Parameter sweep experiment runner for QuadraticRNN and TwoLayerMLP training.
...
def run_single_seed(
    ...
    try:
        ...
        import trained_networks.main as main
...
def _run_experiment_sequential(
    ...
    """Run one experiment configuration and its seeds sequentially.
...
def run_parameter_sweep(sweep_file: str, gpu_ids: list[int | None] | None = None):
    """Run full parameter sweep experiment.

    Args:
        sweep_file: Path to sweep configuration file
        gpu_ids: GPU IDs for execution (for example, ``[0, 1]``). ``[None]``
            selects CPU. If omitted, all available GPUs are detected, with CPU
            used when CUDA is unavailable.
    """
    ...
    if use_parallel:
        ...
        # Sequential execution: run experiments one at a time
        print(f"\n{'=' * 80}")
        print(
        ...
        all_results = {}
        for exp_name, config in experiment_configs:
            results = _run_experiment_sequential(
                exp_name, config, seeds, sweep_dir, gpu_id=gpu_ids[0]
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run parameter sweep experiment for QuadraticRNN or TwoLayerMLP"
    )
    ...
    parser.add_argument(
        ...
        "If omitted, all available GPUs are detected. Parallel execution is enabled when multiple GPUs are used.",
    )
