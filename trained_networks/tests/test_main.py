"""
Tests for trained_networks/main.py
...
Usage:
    pytest trained_networks/tests/test_main.py -v
"""
...
@pytest.fixture
def mock_all_plots():
    """Mock produce_plots and plt.savefig/close to skip visualization entirely."""
    import trained_networks.main as main  # noqa: F401

    with (
        patch("trained_networks.main.produce_plots") as mock_plots,
...
def test_main_c10(temp_run_dir, mock_all_plots):
    ...
    mock_all_plots["produce_plots"].assert_called_once()
...
def test_main_d3(temp_run_dir, mock_savefig):
    ...
    Full integration test: does not mock ``produce_plots``, so the entire
    ...
    This validates the TwoLayerMLP evaluation path in ``produce_plots``,
...
def test_regenerate_plots_cn(temp_run_dir, mock_all_plots):
    ...
    mock_all_plots["produce_plots"].reset_mock()

    main.regenerate_plots(str(temp_run_dir), device="cpu")

    mock_all_plots["produce_plots"].assert_called_once()
    call_kwargs = mock_all_plots["produce_plots"].call_args[1]
