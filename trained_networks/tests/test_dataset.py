"""Tests for trained_networks.dataset."""
...
class TestGroupCompositionDatasetOfflineGroup:
    ...
    def test_default_targets_use_body_frame_order(self, d3_group):
        template = np.zeros(d3_group.order, dtype=np.float32)
        template[0] = 1.0
        ds = dataset.GroupCompositionDataset(
            d3_group,
            template=template,
        ...
        motion = d3_group._N
        row = np.flatnonzero(
            (ds.sequence[:, 0] == current) & (ds.sequence[:, 1] == motion)
        ).item()

        assert int(ds.Y[row].argmax()) == d3_group._cayley(current, motion)

    def test_spatial_targets_remain_available_explicitly(self, d3_group):
        template = np.zeros(d3_group.order, dtype=np.float32)
        template[0] = 1.0
        ds = dataset.GroupCompositionDataset(
            d3_group,
        ...
        assert int(ds.Y[row].argmax()) == d3_group._cayley(motion, current)
